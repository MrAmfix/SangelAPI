    ...
    try:
        ...
        if not user_in_db:
            raise HTTPException(
                ...
                detail=f"Ошибка: пользователь с ID {user_id} не существует",
            )

        if file.content_type not in ["image/jpeg", "image/png"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Недопустимый тип файла. Только .jpeg and .png разрешены.",
            )
        ...
        if file_size > max_file_size_converted:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Размер файла превышает {MAX_FILE_SIZE} МБ.",
            )
        ...
        with Image.open(io.BytesIO(contents)) as img:
            if img.format not in ["JPEG", "PNG"]:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Недпустимый формат. Только JPEG и PNG разрешены",
                )
            img.verify()
        ...
        raise HTTPException(
            ...
            detail=f"Директории '{UPLOAD_FOLDER}' не существует.",
        )
    except OSError as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
